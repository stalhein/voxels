 ...
 * @version 5.1.0
 * @date 2026-01-03
 * @copyright Copyright (c) 2021-2026 Barak Shoshany. Licensed under the MIT
 * license. If you found this project useful, please consider starring it on
 * GitHub! If you use this library in software of any kind, please provide a
 * link to the GitHub repository https://github.com/bshoshany/thread-pool in the
 * source code and documentation. If you use this library in published research,
 * please cite it as follows: Barak Shoshany, "A C++17 Thread Pool for
 * High-Performance Scientific Computing", doi:10.1016/j.softx.2024.101687,
...
// define any feature-test macros.
#ifdef __has_include
...
// `std::jthread` in a C++20 module causes a compilation error. As a workaround,
// until the bug is fixed, the thread pool library automatically falls back to
// `std::thread` if it detects that Clang and libc++ are being used together
// with C++20 modules. This workaround can be disabled by defining
// `BS_THREAD_POOL_DISABLE_WORKAROUNDS` when compiling the module. TODO: Remove
// this workaround when the bug is fixed.
#if defined(__clang__) && defined(_LIBCPP_VERSION) &&                          \
    defined(BS_THREAD_POOL_MODULE) && (__cplusplus >= 202002L) &&              \
    !defined(BS_THREAD_POOL_DISABLE_WORKAROUNDS)
#ifdef __cpp_lib_jthread
#undef __cpp_lib_jthread
#endif
#endif

// At the time of this release, there is a bug when using GCC with libstdc++ on
// Windows via MSYS2 where the `BS.thread_pool` module doesn't compile if both
// native extensions and `import std` are enabled. As a workaround, until the
// bug is fixed, the thread pool library automatically falls back to header
// files if it detects that GCC and libstdc++ are being used together with the
// C++23 `std` module on Windows. This workaround can be disabled by defining
// `BS_THREAD_POOL_DISABLE_WORKAROUNDS` when compiling the module. TODO: Remove
// this workaround when the bug is fixed.
#if (defined(__GNUC__) && defined(_GLIBCXX_RELEASE) && defined(_WIN32)) &&     \
    !defined(BS_THREAD_POOL_DISABLE_WORKAROUNDS)
...
// In GCC with libstdc++ on Linux, loading the system headers after `import std`
// causes compilation errors, so we load them first.
...
#else
#error                                                                         \
    "The thread pool library cannot import the C++ Standard Library as a module using `import std` if the library itself is not imported as a module. Either use `import BS.thread_pool` to import the library, or remove the `BS_THREAD_POOL_IMPORT_STD` macro. Aborting compilation."
#endif
#else
#undef BS_THREAD_POOL_IMPORT_STD
...
// On Linux, <sys/sysmacros.h> defines macros called `major` and `minor`, which
// we undefine here to prevent conflicts.
#ifdef major
#undef major
#endif
#ifdef minor
...
// On Windows, <windows.h> defines macros called `min` and `max`, which we
// undefine here to prevent conflicts.
#ifdef min
#undef min
#endif
#ifdef max
...
/**
 * @brief An enumeration class of flags to be used in the bitmask template
 * parameter of `BS::thread_pool` to enable optional features.
 */
enum class tp : opt_t {
  /**
...
#define BS_THREAD_POOL_DEFINE_BITWISE_OPERATOR(ENUM, OP)                       \
  constexpr ENUM operator OP(const ENUM lhs, const ENUM rhs) noexcept {        \
    return static_cast<ENUM>(static_cast<std::underlying_type_t<ENUM>>(        \
        lhs) OP static_cast<std::underlying_type_t<ENUM>>(rhs));               \
  }                                                                            \
...
/**
 * @brief A simple polyfill for `std::move_only_function`, to be used if C++23
 * features are not available. Note that it does not have all the features of
 * `std::move_only_function`, only the minimum needed for the thread pool
 * library.
 *
...
public:
  ...
  move_only_function(F &&func)
      : ptr(std::make_unique<func_model<std::decay_t<F>>>(
            std::forward<F>(func))) {} // NOLINT(hicpp-explicit-conversions)

  R operator()(Args... args) { return ptr->call(std::forward<Args>(args)...); }
...
private:
  ...
  template <typename F> struct func_model final : func_concept {
    template <typename T, typename = std::enable_if_t<
                              !std::is_same_v<std::decay_t<T>, func_model>>>
 ...
 */
enum pr : priority_t // NOLINT(cppcoreguidelines-use-enum-class) This cannot be
                     // an `enum class` because we need the numerical values.
{
  lowest = -128,
  low = -64,
  normal = 0,
...
struct [[nodiscard]] pr_task {
  ...
  /**
   * @brief The task. It is `mutable` so it can be moved out of the `const`
...
public:
  ...
  blocks(const T first_index_, const T index_after_last_,
      ...
      : num_blocks(num_blocks_), first_index(first_index_),
...
/**
 * @brief A function object class used by `detach_blocks()` and
 * `submit_blocks()` to execute a block function over a specified range of
 * indices.
 *
 ...
 */
template <typename T, typename F, typename R> struct block_task {
  R operator()() { return (*block_ptr)(start, end); }
...
/**
 * @brief A function object class used by `detach_loop()` and `submit_loop()` to
 * execute a loop function over a specified range of indices.
 *
 * @tparam T The type of the indices.
 * @tparam F The type of the function.
 */
template <typename T, typename F> struct loop_task {
  void operator()() {
...
/**
 * @brief A function object class used by `detach_sequence()` and
 * `submit_sequence()` to execute a sequence function over a specified index.
 *
 * @tparam T The type of the index.
 * @tparam F The type of the function.
 * @tparam R The return type of the function (can be `void`).
 */
template <typename T, typename F, typename R> struct sequence_task {
  R operator()() { return (*sequence_ptr)(i); }
...
/**
 * @brief A class that takes a function with a return value (but no arguments),
 * and constructs a task with no return value along with a future used to
 * retrieve the function's return value once the task is executed. Used by
 * `submit_task()` and `submit_bulk()`.
 *
 * @tparam R The return type of the function (can be `void`).
 */
template <typename R> struct task_and_future {
  template <typename F, typename = std::enable_if_t<!std::is_same_v<
                            std::decay_t<F>, task_and_future<R>>>>
  explicit task_and_future(F &&func) {
    std::promise<R> promise;
    future = promise.get_future();
    task = [task = std::forward<F>(func),
 ...
 */
inline bool
set_os_process_affinity([[maybe_unused]] const std::vector<bool> &affinity) {
#if defined(_WIN32)
  DWORD_PTR process_mask = 0;
  for (std::size_t i = 0;
 ...
 * native extensions are enabled, get/set its priority, affinity, or name.
 */
class [[nodiscard]] this_thread {
  template <tp> friend class thread_pool;
...
public:
   ...
   * platform's native API. This should work on Windows and Linux, but is not
   * possible on macOS and Android as the native API does not allow it.
   *
   * @return An `std::optional` object, optionally containing the processor
   ...
   * a value, then the affinity could not be determined. On macOS and Android,
   ...
   * platform's native API. This should work on Windows and Linux, but is not
   * possible on macOS and Android as the native API does not allow it. Note
   * that the thread affinity must be a subset of the process affinity (as
   * obtained using `BS::get_os_process_affinity()`) for the containing process
   * of a thread.
   *
   * @param affinity The processor affinity to set, as an `std::vector<bool>`
   * where each element corresponds to a logical processor.
   * @return `true` if the affinity was set successfully, `false` otherwise. On
   * macOS and Android, this function always returns `false`.
   */
  static bool
  set_os_thread_affinity([[maybe_unused]] const std::vector<bool> &affinity) {
#if defined(_WIN32)
...
struct common_index_type<
  ...
  using type = std::conditional_t<
      ...
      // signed type is 16 bits and the unsigned type is 8 bits, the common type
      // is `std::int16_t`. Otherwise, if both are 16 bits, or the signed type
      // is 32 bits and the unsigned type is smaller, the common type is
      // `std::int32_t`. Otherwise, if both are 32 bits or less, the common type
      // is `std::int64_t`.
      std::conditional_t<
      ...
      // indices explicitly to the desired common type. If the unsigned integer
      // is not 64 bits, then the signed integer must be 64 bits, hence the
      // common type is `std::int64_t`.
...
public:
  ...
  static constexpr bool wait_deadlock_checks_enabled =
      (OptFlags & tp::wait_deadlock_checks) != tp::none;
   ...
   * hyperthreaded, it will count as two threads. If the native extensions are
   * enabled, the pool will instead use the number of threads available to the
   * process, as obtained from `BS::get_os_process_affinity()`, which can be
   ...
   * that it is only called once per block, but it is up to the user to make
   * sure the block function correctly deals with all the indices in each block.
   * Does not return a `BS::multi_future`, so the user must use `wait()` or some
   ...
   * or unsigned integer.
   * @tparam T The common type of the indices, as determined by
   * `BS::common_index_type_t<T1, T2>`.
   ...
   * loop of the form `for (T i = start; i < end; ++i)`. Must not return a
   * value.
  ...
  void detach_blocks(const T1 first_index, const T2 index_after_last, F &&block,
    ...
    enqueue_blocks<T, F, void, false>(
        static_cast<T>(first_index), static_cast<T>(index_after_last),
        std::forward<F>(block), num_blocks, priority);
  }

  /**
   * @brief Submit an iterator range containing functions with no arguments and
   * no return values into the task queue, with the specified priority. To
   * submit functions with arguments, enclose them in lambda expressions. Does
  ...
  /**
   * @brief Submit a container of functions with no arguments and no return
   * values into the task queue, with the specified priority. To submit
   * functions with arguments, enclose them in lambda expressions. Does not
   * return a `BS::multi_future`, so the user must use `wait()` or some other
   * method to ensure that the loop finishes executing, otherwise bad things
   * will happen.
   *
   * @tparam C The type of the container. Must either be an array or have
   ...
   * @brief Parallelize a loop by automatically splitting it into blocks and
   * submitting each block separately to the queue, with the specified priority.
   * The loop function takes one argument, the loop index, and it is called
   * exactly once per index, but many times per block. Does not return a
   * `BS::multi_future`, so the user must use `wait()` or some other method to
   * ensure that the loop finishes executing, otherwise bad things will happen.
   *
   * @tparam T1 The type of the first index. Should be a signed or unsigned
   * integer.
   * @tparam T2 The type of the index after the last index. Should be a signed
   ...
   * index_after_last; ++i)`. Note that if `index_after_last <= first_index`, no
   * tasks will be submitted.
   * @param loop A function that will be called once per index, many times per
   * block. Should take exactly one argument: the loop index. Must not return a
   * value.
   * @param num_blocks The maximum number of blocks to split the loop into. The
   * default is 0, which means the number of blocks will be equal to the number
   * of threads in the pool.
  ...
  void detach_loop(const T1 first_index, const T2 index_after_last, F &&loop,
                   const std::size_t num_blocks = 0,
                   const priority_t priority = 0) {
    enqueue_loop<T, F, false>(static_cast<T>(first_index),
                              static_cast<T>(index_after_last),
                              std::forward<F>(loop), num_blocks, priority);
  }
   ...
   * @param sequence A function that will be called once per index. Should take
   * exactly one argument, the index. Must not return a value.
   * @param priority The priority of the tasks. Should be between -128 and +127
   * (a signed 8-bit integer). The default is 0. Only taken into account if the
   * flag `BS::tp::priority` is enabled in the template parameter, otherwise has
   * no effect.
  ...
  void detach_sequence(const T1 first_index, const T2 index_after_last,
    ...
    return enqueue_sequence<T, F, void, false>(
        static_cast<T>(first_index), static_cast<T>(index_after_last),
        std::forward<F>(sequence), priority);
  }
   ...
   * tasks out of the queue, although any tasks already executing will keep
  ...
  /**
   * @brief Reset the pool with the default number of threads (as if constructed
   * with the default constructor). Waits for all tasks to be completed, both
   * running and queued, then destroys the thread pool and creates a new one
   * with an empty task queue. If pausing is enabled, only waits for tasks that
   * are currently running before destroying the pool; once the pool is reset,
   * it will then resume executing the tasks that remained in the queue and any
   * newly submitted tasks. If the pool was paused before resetting it, the new
  ...
  /**
   * @brief Reset the pool with a new number of threads. Waits for all tasks to
   * be completed, both running and queued, then destroys the thread pool and
   * creates a new one with an empty task queue. If pausing is enabled, only
   * waits for tasks that are currently running before destroying the pool; once
   * the pool is reset, it will then resume executing the tasks that remained in
   * the queue and any newly submitted tasks. If the pool was paused before
  ...
  /**
   * @brief Reset the pool with the default number of threads and a new
   * initialization function. Waits for all tasks to be completed, both running
   * and queued, then destroys the thread pool and creates a new one with an
   * empty task queue. If pausing is enabled, only waits for tasks that are
   * currently running before destroying the pool; once the pool is reset, it
   * will then resume executing the tasks that remained in the queue and any
   * newly submitted tasks. If the pool was paused before resetting it, the new
   * pool will be paused as well.
   *
   * @param init An initialization function to run in each thread before it
   ...
   * function. Waits for all tasks to be completed, both running and queued,
   * then destroys the thread pool and creates a new one with an empty task
   * queue. If pausing is enabled, only waits for tasks that are currently
   * running before destroying the pool; once the pool is reset, it will then
   * resume executing the tasks that remained in the queue and any newly
   * submitted tasks. If the pool was paused before resetting it, the new pool
   ...
   * that it is only called once per block, but it is up to the user to make
   * sure the block function correctly deals with all the indices in each block.
   * If the block function has a return value, get a `BS::multi_future` for the
   * eventual returned values. If the block function has no return value, get a
   * `BS::multi_future<void>` which can be used to wait until all the tasks
   * finish.
   *
   * @tparam T1 The type of the first index. Should be a signed or unsigned
   * integer.
   ...
   * exactly two arguments: the first index in the block and the index after the
   * last index in the block. `block(start, end)` should typically involve a
   * loop of the form `for (T i = start; i < end; ++i)`. Can return a value.
   * @param num_blocks The maximum number of blocks to split the loop into. The
   * default is 0, which means the number of blocks will be equal to the number
   * of threads in the pool.
   * @param priority The priority of the tasks. Should be between -128 and +127
  ...
  submit_blocks(const T1 first_index, const T2 index_after_last, F &&block,
                const std::size_t num_blocks = 0,
                const priority_t priority = 0) {
    return enqueue_blocks<T, F, R, true>(
        static_cast<T>(first_index), static_cast<T>(index_after_last),
        std::forward<F>(block), num_blocks, priority);
  }

  /**
   * @brief Submit an iterator range containing functions with no arguments into
   * the task queue, with the specified priority. To submit functions with
   * arguments, enclose them in lambda expressions. If the functions have return
   * values, get a `BS::multi_future` for the eventual returned values. If the
   * functions have no return values, get a `BS::multi_future<void>` which can
   * be used to wait until all the tasks finish.
   *
   * @tparam I The type of the iterators.
   * @tparam F The type of the functions.
   * @tparam R The return type of the functions (can be `void`, but must be the
   ...
   * no effect.
   * @return A `BS::multi_future` that can be used to wait for all the tasks to
   * finish. If the functions return values, the `BS::multi_future` can also be
   * used to obtain the values returned by each task.
   */
  template <typename I, typename F = decltype(*std::declval<I>()),
            typename R = std::invoke_result_t<std::decay_t<F>>>
  [[nodiscard]] multi_future<R> submit_bulk(const I first, const I last,
                                            const priority_t priority = 0) {
    if (first != last) {
  ...
  /**
   * @brief Submit a container of functions with no arguments into the task
   * queue, with the specified priority. To submit functions with arguments,
   * enclose them in lambda expressions. If the functions have return values,
   * get a `BS::multi_future` for the eventual returned values. If the functions
   * have no return values, get a `BS::multi_future<void>` which can be used to
   ...
   * used to obtain the values returned by each task.
   */
  template <typename C, typename F = decltype(*std::declval<C &>().begin()),
            typename R = std::invoke_result_t<std::decay_t<F>>>
  [[nodiscard]] multi_future<R> submit_bulk(C &container,
                                            const priority_t priority = 0) {
   ...
   * submitting each block separately to the queue, with the specified priority.
   * The loop function takes one argument, the loop index, and it is called
   * exactly once per index, but many times per block. Returns a
   * `BS::multi_future` which can be used to wait until all the tasks finish.
   *
   * @tparam T1 The type of the first index. Should be a signed or unsigned
   * integer.
   * @tparam T2 The type of the index after the last index. Should be a signed
   ...
   * @brief Submit a sequence of tasks enumerated by indices to the queue, with
   * the specified priority. The sequence function takes one argument, the task
   * index, and will be called once per index. If the sequence function has a
   * return value, get a `BS::multi_future` for the eventual returned values. If
   * the sequence function has no return value, get a `BS::multi_future<void>`
   * which can be used to wait until all the tasks finish.
   *
   * @tparam T1 The type of the first index. Should be a signed or unsigned
   * integer.
   * @tparam T2 The type of the index after the last index. Should be a signed
   ...
   * will be returned.
   * @param sequence A function that will be called once per index. Should take
   * exactly one argument, the index. Can return a value.
   * @param priority The priority of the tasks. Should be between -128 and +127
   * (a signed 8-bit integer). The default is 0. Only taken into account if the
   * flag `BS::tp::priority` is enabled in the template parameter, otherwise has
   * no effect.
  ...
  submit_sequence(const T1 first_index, const T2 index_after_last, F &&sequence,
                  const priority_t priority = 0) {
    return enqueue_sequence<T, F, R, true>(static_cast<T>(first_index),
                                           static_cast<T>(index_after_last),
                                           std::forward<F>(sequence), priority);
  }
...
private:
   ...
   * number of threads. If the parameter is zero, or a parameter was not
   ...
   * If the native extensions are enabled, the pool will instead use the number
   * of threads available to the process, as obtained from
   * `BS::get_os_process_affinity()`, which can be less than the number of
   * hardware threads.
   */
  [[nodiscard]] static std::size_t determine_thread_count(
      const std::size_t num_threads) noexcept(!thread_pool_native_extensions) {
    if (num_threads > 0)
      return num_threads;
#ifdef BS_THREAD_POOL_NATIVE_EXTENSIONS
    const std::optional<std::vector<bool>> affinity =
        BS::get_os_process_affinity();
    if (affinity.has_value()) {
      const std::size_t affinity_thread_count = static_cast<std::size_t>(
          std::count(affinity->begin(), affinity->end(), true));
   ...
   * @tparam F The type of the block function.
   * @tparam R The return type of the block function (can be `void`).
   * @tparam submit `true` if called from `submit_blocks()`, `false` if called
   * from `detach_blocks()`.
   ...
   * @return A `BS::multi_future` if `submit` is `true`, or `void` if `submit`
   * is `false`.
   */
  template <typename T, typename F, typename R, bool submit,
            typename N = std::conditional_t<submit, multi_future<R>, void>>
  [[nodiscard]] N enqueue_blocks(const T first_index, const T index_after_last,
                                 F &&block, std::size_t num_blocks,
                                 const priority_t priority = 0) {
    if (index_after_last > first_index) {
      ...
      for (std::size_t i = 0; i < num_blocks; ++i)
        all_tasks.emplace_back(
            block_task_t{block_ptr, blks.start(i), blks.end(i)});
      if constexpr (submit)
   ...
   * @tparam T The type of the indices.
   * @tparam F The type of the loop function.
   * @tparam submit `true` if called from `submit_loop()`, `false` if called
   * from `detach_loop()`.
   * @tparam N The return type of this helper function.
   * @param first_index The first index in the loop.
   * @param index_after_last The index after the last index in the loop.
   ...
   * is `false`.
   */
  template <typename T, typename F, bool submit,
            typename N = std::conditional_t<submit, multi_future<void>, void>>
  [[nodiscard]] N enqueue_loop(const T first_index, const T index_after_last,
                               F &&loop, std::size_t num_blocks,
                               const priority_t priority = 0) {
    if (index_after_last > first_index) {
      ...
      for (std::size_t i = 0; i < num_blocks; ++i)
        all_tasks.emplace_back(
            loop_task_t{loop_ptr, blks.start(i), blks.end(i)});
      if constexpr (submit)
        return submit_bulk(all_tasks, priority);
      else
        detach_bulk(all_tasks, priority);
   ...
   * @tparam F The type of the sequence function.
   * @tparam R The return type of the sequence function (can be `void`).
   * @tparam submit `true` if called from `submit_sequence()`, `false` if called
   * from `detach_sequence()`.
   * @tparam N The return type of this helper function.
   * @param first_index The first index in the sequence.
  ...
  template <typename T, typename F, typename R, bool submit,
            typename N = std::conditional_t<submit, multi_future<R>, void>>
  [[nodiscard]] N enqueue_sequence(const T first_index,
                                   const T index_after_last, F &&sequence,
                                   const priority_t priority = 0) {
    if (index_after_last > first_index) {
      ...
      std::vector<std::conditional_t<submit, sequence_task_t, task_t>>
          all_tasks;
      all_tasks.reserve(
          static_cast<std::size_t>(index_after_last - first_index));
  ...
  void worker(BS_THREAD_POOL_WORKER_TOKEN const std::size_t idx) {
    ...
    while (true) {
      ...
      {
        task_t task =
            pop_task(); // NOLINT(misc-const-correctness) In C++23 this cannot
                        // be const since `std::move_only_function::operator()`
 ...
 */
   ...
   * although any tasks already executing will keep running until they are
...
/**
 * @brief A utility class to synchronize printing to one or more output streams
...
public:
  ...
  /**
   * @brief Print any number of items into each output stream. Ensures that no
   * other threads print to the streams simultaneously, as long as they all
  ...
  /**
   * @brief Print any number of items into each output stream, followed by a
   * newline character. Ensures that no other threads print to the streams
